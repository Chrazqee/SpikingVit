...
LoaderDataDictGenX = Dict[DataType, Union[List[torch.Tensor], ObjectLabels, SparselyBatchedObjectLabels, List[bool]]]

LstmState = Optional[Tuple[torch.Tensor, torch.Tensor]]
LstmStates = List[LstmState]

SnnState = Optional[Tuple[torch.Tensor, torch.Tensor]]
SnnStates = List[SnnState]

FeatureMap = torch.Tensor
BackboneFeatures = Dict[int, torch.Tensor]

EncoderFeatures = Dict[int, torch.Tensor]
