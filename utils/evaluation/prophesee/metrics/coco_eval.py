...
def evaluate_detection(gt_boxes_list, dt_boxes_list, classes=("car", "pedestrian"), height=240, width=304,
    ...
    Args:
        gt_boxes_list: list of numpy array for GT boxes (one per file)
        dt_boxes_list: list of numpy array for detected boxes
        classes: iterable of classes names
        height: int for box size statistics
        width: int for box size statistics
        time_tol: int size of the temporal window in micro seconds to look for a detection around a gt box
        return_aps:
    """
    flattened_gt = []
    flattened_dt = []
