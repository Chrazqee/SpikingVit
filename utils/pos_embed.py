...
def build_sincos2d_pos_embed(
    ...
    grid = torch.stack(
        torch.meshgrid([torch.arange(s, device=device, dtype=dtype) for s in feat_shape], indexing='xy')).flatten(1).transpose(0, 1)
...
if __name__ == "__main__":
    x = torch.randn(1, 2).to('cuda:1')
    device = x.device
    pos_embed = build_sincos2d_pos_embed([12, 20], dim=512, temperature=10000., interleave_sin_cos=False, device=device)
    print(pos_embed.shape)
    print(pos_embed)
    pos_embed = pos_embed.transpose(-2, -1).view(512, 12, 20).unsqueeze(dim=0).repeat(4, 1, 1, 1)
    print(pos_embed.shape)
